...
class MusicTokenizer(Tokenizer):
    ...
    class Duration(Tokenizer.Item):
        rx = ly.rx.duration
        def __init__(self, matchObj, tokenizer):
            self.duration = matchObj.group('duration')
            self.dur = matchObj.group('dur') or ''
            self.dots = matchObj.group('dots') or ''
            self.scale = matchObj.group('scale') or ''
	
    class PitchDuration(Tokenizer.Item):
		rx = ly.rx.pitchdur
		def __init__(self, matchObj, tokenizer):
			self.step = matchObj.group('step')
			self.octave = matchObj.group('octave') or ''
			self.cautionary = matchObj.group('cautionary') or ''
			self.octcheck = matchObj.group('octcheck') or ''
			self.duration = matchObj.group('duration') or ''
			self.dur = matchObj.group('dur') or ''
			self.dots = matchObj.group('dots') or ''
			self.scale = matchObj.group('scale') or ''
